...
export default function Sidebar({ collapsed = false, mobileOpen = false, onClose, isMobile = false }: Props) {
  ...
  if (isMobile) {
    return (
      ...
      <div className={`fixed inset-0 z-50 flex ${mobileOpen ? '' : 'pointer-events-none'}`}>
        <div
          className={`w-64 bg-white p-6 border-r border-gray-100 h-full transform transition-transform duration-300 ease-in-out ${
              ...
              {items.map((it) => {
                ...
                return (
                  <li key={it.href}>
                    ...
                    >
                      <span className={`inline-flex w-10 h-10 items-center justify-center rounded-md ${active ? 'bg-[var(--primary)] text-white' : 'text-gray-600'}`}>
                        {it.icon}
                      </span>
                      {it.label}
                    </Link>
  ...
  return (
    <aside
      className={`bg-white h-full min-h-screen p-6 border-r border-gray-100 flex flex-col transition-all duration-200 ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      ...
      <nav className="flex-1">
        <ul className="space-y-2">
          {items.map((it) => {
            ...
            const paddingWhen = collapsed ? 'justify-center px-0' : 'px-4';
            return (
              <li key={it.href}>
                <Link
                  href={it.href}
                ...
                >
                  <span className={`inline-flex w-10 h-10 items-center justify-center rounded-md ${active ? 'bg-[var(--primary)] text-white' : 'text-gray-600'}`} style={{fontSize:18}}>
                    {it.icon}
                  </span>
                  {!collapsed && it.label}
                </Link>
