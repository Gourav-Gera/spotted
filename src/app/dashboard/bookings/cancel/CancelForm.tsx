...
export default function CancelForm(){
  ...
  return (
    <form onSubmit={onSubmit} className="card-surface">
      <div className="mb-4">
        <label className="block text-sm mb-2">Select reason</label>
        <select value={reason} onChange={(e)=>setReason(e.target.value)} className="w-full border rounded-md p-3">
      ...
      <div className="mb-6">
        ...
        <textarea value={note} onChange={(e)=>setNote(e.target.value)} className="w-full border rounded-md p-3 h-36" />
      </div>
