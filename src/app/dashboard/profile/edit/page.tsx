...
export default function EditProfilePage(){
  ...
  return (
    <div className="p-8">
      <h1 className="text-[26px] leading-tight font-semibold mb-10">Edit Profile</h1>
      <form onSubmit={onSubmit} className="max-w-xl space-y-7">
        ...
        <div>
          ...
          <input value={name} onChange={e=>setName(e.target.value)} className="w-full rounded-full bg-white border border-gray-200 px-6 py-3.5 text-base focus:outline-none focus:ring-2 focus:ring-[#4A5D52]/25" />
        </div>
        <div>
          <label className="block text-[13px] tracking-wide font-semibold mb-2">Email</label>
          <input value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-full bg-white border border-gray-200 px-6 py-3.5 text-base focus:outline-none focus:ring-2 focus:ring-[#4A5D52]/25" />
        </div>
