        ...
        <div className="card-surface p-0">
          <div className="flex items-center justify-between gap-4 mb-6 rounded-full border border-[#E5E5E5] overflow-hidden">
            {[
              ...
              <button key={t.key} onClick={()=>setTab(t.key as any)} className={`pill-tab w-[25%] ${tab===t.key ? 'bg-[var(--primary)] text-white' : 'bg-[#F6F7F9] text-[var(--gray)]'}`}>{t.label}</button>
            ))}
          </div>
          <div className="flex items-center gap-2 bg-gray-50 rounded-full px-3 py-2 search-pill">
            ...
            <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search orders..." className="w-full text-sm" />
          </div>
                  ...
                  <tbody>
                    {filtered.map(r => (
                      <tr key={r.id} className="align-middle">
                        ...
                        <td className="flex items-center justify-end gap-4 py-4">
